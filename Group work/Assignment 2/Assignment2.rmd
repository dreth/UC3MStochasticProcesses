...
test <- data.frame(val=aa, tt=tt)
plot(test$tt, main='Probando entre 0 y 200')
# trying 139-140
tt = c()
aa = seq(139,140,0.01)
for (i in aa) {
...
test <- data.frame(val=aa, tt=tt)
plot(test$tt, main='Probando entre 139 y 140')
```
...
non_hom_poisson <- function(fun,l,a,b,start=0) {
    ...
    # Homogenous poisson process arrival times
    val <- rpois(1,l*(b-a))
    intervals <- (b-a) * sort(runif(val)) + a

    # Non-homogenous poisson process
    evs <- length(intervals)
    nh_val <- 0 + start
    nh_intervals <- c()
    for (i in 1:evs) {
    ...
    nh_events <- seq(1+start,nh_val,1)
...
simulation <- function(iters, functions, lambdas, ints) {
    ...
    for (i in 1:iters) {
        ...
        events <- c()
        for (k in 1:4) {
            int <- non_hom_poisson(lambda_funs[[k]],lambdas[[k]],ints[[k]][1],ints[[k]][2],start=maximum)
            maximum <- max(int$events)
        ...
        # plots
        if (i == 1) {plot(arr_times, events, cex=0.5, pch='.', col=randomColor(), xlim=c(0,1.5), ylim=c(0,150))}
...
# lambdas
lambda_1 <- 100
lambda_2 <- mean(600*seq(1/2,3/4,0.001)-200)
lambda_3 <- mean(400*seq(3/4,1,0.001)-50)
lambda_4 <- mean(-500*seq(1,1.5,0.001)+850)
...
data <- simulation(100, lambda_funs, lambdas, ints)
```
...
for (i in 0:143) {
    ...
    print((exp(-140) * 140^i)/(factorial(i)))
}
test
```
